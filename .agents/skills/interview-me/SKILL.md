...
When running inside a pi session with this extension installed:

- **Sub-agent delegation:** For deep research before interviewing (e.g., reading an existing codebase to inform questions), use `Agent({ subagent_type: "General-Purpose-Medium", prompt: "Research the current state of <topic> in this codebase. Return: existing patterns, related modules, constraints, prior decisions.", description: "Pre-interview research" })`. Use `-Low` for fast scans, `-High` for deep architectural analysis.
- **Context loading:** Before asking, run `/init-context` to load `AGENTS.md` rules so the interview reflects project conventions.
- **WebFetch:** This skill does not need to fetch web pages; the interview is about user intent, not external facts. If research is needed, delegate to a sub-agent (above) or run a web search through bash.
- **File output:** When intent is confirmed, write to `docs/intent/[topic].md` directly via `write` tool (do not delegate to sub-agents for this — it should stay in the main conversation so the user sees it being written).
...
- **`spec-driven-development`**: downstream. If the confirmed intent is concrete ("I want X for Y users with Z success criteria"), hand off to `spec-driven-development` to write it down. In a pi session, you can also run the `/spec` command to generate a `SPEC.md` template automatically.
- **`planning-and-task-breakdown`**: two hops downstream of this skill (after the spec). Use `/plan` to break it into tasks.
